...
function draw(){const w=simCanvas.width,h=simCanvas.height;ctx.clearRect(0,0,w,h);const g=ctx.createRadialGradient(w*.5,h*.45,40,w*.5,h*.45,w*.75);g.addColorStop(0,'#13233c');g.addColorStop(.58,'#0f172a');g.addColorStop(1,'#030712');ctx.fillStyle=g;ctx.fillRect(0,0,w,h);if($('showGrid').checked)drawGrid();drawSoftRegions();if($('showConnections').checked){ctx.save();ctx.strokeStyle='rgba(248,113,113,.24)';ctx.lineWidth=1;for(const [x1,y1,x2,y2] of state.contactsToDraw){ctx.beginPath();ctx.moveTo(x1,y1);ctx.lineTo(x2,y2);ctx.stroke()}ctx.restore()}drawTrails();drawAgents();drawCanvasBadge()}
...
function drawChart(){const w=chartCanvas.width,h=chartCanvas.height,p=params();cctx.clearRect(0,0,w,h);cctx.fillStyle='#030712';cctx.fillRect(0,0,w,h);const l=45,r=18,t=20,b=28,pw=w-l-r,ph=h-t-b;cctx.strokeStyle='rgba(255,255,255,.08)';cctx.lineWidth=1;for(let i=0;i<=5;i++){const y=t+ph/5*i;cctx.beginPath();cctx.moveTo(l,y);cctx.lineTo(w-r,y);cctx.stroke();cctx.fillStyle='rgba(203,213,225,.65)';cctx.font='11px Arial';cctx.fillText(String(Math.round(p.population*(1-i/5))),8,y+4)}if(state.history.length<2)return;line('S','#60a5fa');line('E','#f59e0b');line('I','#ef4444');line('R','#22c55e');cctx.fillStyle='#cbd5e1';cctx.font='12px Arial';cctx.fillText('dias',w-50,h-8);function line(k,col){cctx.beginPath();state.history.forEach((d,i)=>{const x=l+i/Math.max(1,state.history.length-1)*pw,y=t+ph-d[k]/p.population*ph;if(i===0)cctx.moveTo(x,y);else cctx.lineTo(x,y)});cctx.strokeStyle=col;cctx.lineWidth=2.8;cctx.stroke()}}
